...
A Shimatzu Autograph AG-X universal testing machine with 10 kN load cell was
used to carry out static tests. Initial tests were conducted on dummy wood specimens in order to find the approximate elastic loading range for all specimen orientations. The compression test was repeated twice for each specimen in each L, R and T-direction (cf. Fig. 1). Both loading and unloading cycles were recorded as shown in the example of Figure 2. The Young’s modulus was determined from the unloading part. The main reason for using the unloading curve is that slight surface irregularities of the sample or loading platen is likely to induce local plasticity during the loading part, and an increase of the contact area between the sample and the platen. During the unloading, these effects should be stabilized and the contact area should be close to the whole surface area reached at the maximum load of elastic region. The conventional way to measure strains is by using the built in measurement system or strain gages. Nowadays, full field strain measurement techniques such as digital speckle photography (DSP) (Sjödahl and Synnergren 1999) or 3D/stereo-digital image correlation (DIC) (Majano-Majano et al. 2012) are used more and more. The strain measurement using DIC has shown a good performance in comparison to the traditional strain gauges (Huang et al. 2010; Xavier et al. 2012). In our case, full-field displacements were observed with a digital-image correlation (DIC) equipment GOM Aramis stereo system 5M. The distance to the measured object was 300 mm. Each surface of specimen was spray painted with speckles for better contrast. The applied force values were continuously recorded by the DIC system during testing, and stored together with the sampled images. The displacement rate was 0.5 mm per minute. The ambient conditions were 23°C and 51% relative humidity. All tests were performed without delay after the samples were removed from the conditioning storage container, so that the moisture content would essentially be the same as after conditioning. The measurement sampling frequency was 1 Hz. Every frame that was recorded by the DIC system was compared with the undeformed state in order to calculate the displacement and strain fields. The area of interest used in image analysis was a centred square of 400 mm2, where a uniform deformation field was found. The acquired strain fields were smoothed by averaging of each data pixel with 3 × 3 surrounding pixels.
The strain determined by DIC in the loading direction was plotted against the applied stress, see Figure 2. 
Poisson’s ratios νij for all corresponding orthotropic planes were calculated as a negative ratio between the average transverse εjj (passive) and normal εii (compressive and active) strains. The average transverse and normal strains measured along with normal ones were acquired with DIC technique. 
...
\caption{\label{fig:experiment} DIC image of \textit{Vasa} oak specimen during
compressive testing including displacement field (mm).

\textit{\color {red} Place the figure 2 here as well}}
...
\caption{Experimental strain vs. stress curves
for \textit{Vasa} oak  specimen, chosen surface area of interest(\ref{ex1}) and
...
which corresponds to approximate 1\%  of average strain respectively.
...
$E_{1} = E_{2} = a \cdot E_{3}$, where \textit{a} varies from 0,1 (transverse
isotropy) to 1 (isotropy).The same was done for the Poisson's ratio and shear
modulus.
