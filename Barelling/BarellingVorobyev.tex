...
has a unity value and Poisson's ratio $\nu = 0.25$. For transverse
isotropic material the stiffness parameters were taken as engineering notation of
compliance matrix. For simplification the stiffness coefficients $E_{3}$ and
$E_{1} = E_{2} = E_{3}/a$, where coefficient \textit{a} varies from 1 (isotropy)
to 10 (transverse isotropy). 

The orthotropic material was simulated with the imput parameters obtained from
the mechanical experiments that are presented in Table \ref{table:nonlin}.
