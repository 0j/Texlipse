...
\begin{axis}[name=b4,at={($(b1.south)-(0,1.5cm)$)},anchor=north,
height=4cm,width=4cm, title={a')},
%    xlabel={Temperature [\textcelsius]},
    ylabel={Error Poisson's ratio},
    xmin=0.05, xmax=0.45,
    ymin=0.75, ymax=1.01,
...
\begin{axis}[
	name=b5,at={($(b2.south)-(0,1.5cm)$)},anchor=north, height=4cm, width=4cm,
    title={b')},
...
different given Poisson's ratio ($\nu_{12}$) values for a) choen area of
interest, total area of interest c) experimental methodology.
a') Simulated Poisson's ratio error values of material with different given
Poisson's ratio ($\nu_{12}$) values for choen area of
...
Poisson's ratio and the $K$ factor. The general trend is that with the increasing Poisson's ratio
$\nu_{12}$ value, the error is increasing. However the error for the istropic
material is significantly larger when factor $K=1$. The error in Young's moduli
for the transverse isotropic material, is less in the axial direction 3
than in transverse direction 1 (Figure \ref{fig:isovstriso} b,d). This behaviour
can be discussed with the help of Figure \ref{fig:Error}b and b'. For the
isotropic material the severe influence of barrelling occurs due to the
isotropic parameters in all planes. Whereas in transverse-isotropic material,
the influence of barelling is spread less in the surface area. Additionally the
effect is less severe due to material anisotropy.
\begin{description}
...
\item What were the findings?
	...
	\color{black}
	    ...
	    add the plots}
	    \item 3 Maybe add the transverse figures for the Error plot figures
	    \item Change figure 4 b' and a'
