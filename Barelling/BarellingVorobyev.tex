...
the experiments. The geometry can actually be 

For precious wood material, e.g. archaeological wood, it is frequently better to use cubic samples loaded in compression, since the same sample can be loaded elastically by applying load consecutively in all three directions (Ljungdahl and Berglund 2007). 
The precious material can then be saved to a larger extent and the measurements are done on the same sample. Disadvantages with compressive testing of cubes compared to tensile testing of dog-bone specimens, 
is that obtaining a pure compressive stress state is more complicated due to inhomogeneous distribution of applied compression force due to geometrical imperfections of test specimen (Toftegaard 1999).
Moreover, barrelling formation in compressive loading, due to the more or less restrained contact slip on the platens area, makes it difficult to assess the transverse deformations without having to resort to finite element modelling. 
The Poisson’s ratios are thus more difficult to obtain directly from the kind of loading. Strain field measurements using digital image correlation are often required to estimate the stiffness parameters accurately (Dahl and Malo 2009; Majano-Majano et al. 2012; Xavier et al. 2012; Ozyhar et al. 2013). 
Additionally, the cylindrical orthotropy and curvature of the annual rings make the stress field even more non-uniform for large specimens. For too small specimens, the tested volume might not be representative, leading to a large scatter in measured stiffness. 
...
Moreover, this test is relatively easy to carry out and one sample can be used for all three directions, if loading is performed only in the elastic reversible region. 
...
The elastic, ultimate and also creep properties of the Vasa oak are needed in
stress analysis and design of improved support structure for the ship. 
...
Both the aging (Bjurhager et al. 2012) and the PEG impregnation (Ljungdahl and Berglund 2007; Bjurhager et al. 2010) affect the mechanical properties. 
